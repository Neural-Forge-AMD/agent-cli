/**
 * Cloud Web Search & Live Docs MCP Server - JSON-RPC 2.0 stdio server implementation.
 * Aligned with MCP Specification (2024-11-05).
 */

import { createInterface } from "node:readline";
import { getWebSearchToolSchemas, executeWebSearchTool } from "./tools";
import type { JsonRpcRequest, JsonRpcResponse } from "../../types";

export class WebSearchMcpServer {
  private tools = getWebSearchToolSchemas();
  ...
  async handleRequest(req: JsonRpcRequest): Promise<JsonRpcResponse | null> {
    ...
    if (method === "tools/call") {
      ...
      try {
        const output = await executeWebSearchTool(toolName, args);
        return {
          jsonrpc: "2.0",
          id,
          result: {
...
if (import.meta.main) {
  ...
  server.start().catch((err) => {
    console.error("Failed to start Web Search MCP Server:", err);
    process.exit(1);
  });
}
