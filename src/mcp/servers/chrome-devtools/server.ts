/**
 * Chrome DevTools MCP Server - JSON-RPC 2.0 stdio server implementation.
 * Aligned with MCP Specification (2024-11-05).
 */

import { createInterface } from "node:readline";
import { ChromeDevToolsController } from "./controller";
import { getChromeDevToolsToolSchemas, executeChromeDevToolsTool } from "./tools";
import type { JsonRpcRequest, JsonRpcResponse } from "../../types";

export class ChromeDevToolsMcpServer {
  private controller = new ChromeDevToolsController();
  private tools = getChromeDevToolsToolSchemas();
  ...
  async handleRequest(req: JsonRpcRequest): Promise<JsonRpcResponse | null> {
    ...
    if (method === "tools/call") {
      ...
      try {
        const output = await executeChromeDevToolsTool(this.controller, toolName, args);
        return {
          jsonrpc: "2.0",
          id,
          result: {
...
if (import.meta.main) {
  ...
  server.start().catch((err) => {
    console.error("Failed to start Chrome DevTools MCP Server:", err);
    process.exit(1);
  });
}
